...
foreach ($arResult["ITEMS"] as $item) {
    ...
    if ($item["PROPERTIES"]["FORMAT_FOTO"]['VALUE_ENUM_ID'] == 62 ) {
        if ($item["PROPERTIES"]["TEXT_FORMAT"]['VALUE_ENUM_ID'] == 65) {
            ?>
            <section id="<?= $this->GetEditAreaId($item['ID']); ?>" class="mpage-banner full-banner center-style">
                <div class="banner-slide lazyloaded <?=$whiteSlide?>"
                     data-bgset="<?= $srcMob ?> [(max-width: 575px)] | <?= $srcPlan ?> [(max-width: 1068px)] | <?= $srcPK ?> [(min-width: 1069px)]"
        ...
        if ($item["PROPERTIES"]["FORMAT_FOTO"]['VALUE_ENUM_ID'] == 64) {
            echo "reverse-style";
        }
        ?>">
