...
?>">
    <form name="<? echo $arResult["FILTER_NAME"] . "_form" ?>" action="<? echo $arResult["FORM_ACTION"] ?>"
        ...
        <span class="filter-form__title">Фильтры</span>
        <div class="filter-form__checked">
            <span class="checked-value">Рубашки</span>
            <span class="checked-value">Свитера</span>
            <span class="checked-value">Куртки</span>
            <span class="checked-value">M</span>
            <span class="checked-value">Коричневый</span>
        </div>
...
//                )
                ...
                switch ($arItem["NAME"]) {
                    case "Цвет":
                        ?>
                       <div class="filter-form__section <? if ($arItem["DISPLAY_EXPANDED"] == "Y"): ?> active<? endif ?>">
                            <a href="javascript:void(0);" class="section-name"><i
                                        class="icon icon-select_arrow"></i> <?= $arItem["NAME"] ?></a>
                            <div class="section-list">
                                ...
                                <? foreach ($arItem["VALUES"] as $val => $ar) {
                                    ...
                                    <label for="<?= $ar["CONTROL_ID"] ?>"
                                        ...
                                        <input class="color-input" type="checkbox" name="<?= $ar["CONTROL_NAME"] ?>"
                                               id="<?= $ar["CONTROL_ID"] ?>"
                                               value="<?= $ar["HTML_VALUE"] ?> <? echo $ar["CHECKED"] ? 'checked="checked"' : '' ?>">
                                        <span class="color-name"><span style="background-color: #D0C4A4;

                                                    background-image:url('<?= $ar["FILE"]["SRC"] ?>');
                    ...
                    case "Размеры одежды ":
                        ...
                        <div class="filter-form__section <? if ($arItem["DISPLAY_EXPANDED"] == "Y"): ?> active<? endif ?>">
                            <a href="javascript:void(0);" class="section-name"><i
                                        class="icon icon-select_arrow"></i> <?= $arItem["NAME"] ?></a>
                            <div class="section-list size-list">
                                <a href="javascript:void(0);" class="size-link"><i class="icon icon-info"></i> <span>Как определить свой размер?</span></a>
                                <?
                                foreach ($arItem["VALUES"] as $val => $ar) {
                                    ?>
                                    <div class="input-group size-group">
                                        <label data-role="label_<?= $ar["CONTROL_ID"] ?>"
                                            ...
                                            <span class="size-name"><?= $ar["VALUE"]; ?></span>
                                        </label>
                                    </div>
                                    <?
                                } ?>
        ...
        <a id="filter-close-btn" href="javascript:void(0);" class="filter-close-btn btn-fill-style">Закрыть фильтры</a>
    </form>


    <form style="display: none" class="bx-filter <?= $templateData["TEMPLATE_CLASS"] ?>" name="<? echo $arResult["FILTER_NAME"] . "_form" ?>" action="<? echo $arResult["FORM_ACTION"] ?>"
          method="get" class=" filter-form">
        <span class="filter-form__title">Фильтры</span>
        <div class="filter-form__checked">
            <span class="checked-value">Рубашки</span>
        ...
        <div class="">
            <? foreach ($arResult["ITEMS"] as $key => $arItem)//prices
            {
                ...
                if (isset($arItem["PRICE"])):
                    ...
                    <div class="bx-filter-parameters-box filter-form__section bx-active active">
                        <span class="bx-filter-container-modef"></span>
                        <a class="section-name" onclick="smartFilter.hideFilterProps(this)">

                            <i data-role="prop_angle"
                               class="icon icon-select_arrow fa fa-angle-<? if ($arItem["DISPLAY_EXPANDED"] == "Y"):?>up<? else:?>down<?endif ?>"></i>
            ...
            foreach ($arResult["ITEMS"] as $key => $arItem) {
                ...
                <div class="bx-filter-parameters-box filter-form__section <? if ($arItem["DISPLAY_EXPANDED"] == "Y"):?>bx-active active<?endif ?>">
                    <span class="bx-filter-container-modef"></span>
                    <a href="javascript:void(0);" class="bx-filter-parameters-box-title section-name" onclick="smartFilter.hideFilterProps(this)">
                        <i data-role="prop_angle icon icon-select_arrow"
                           class="fa fa-angle-<? if ($arItem["DISPLAY_EXPANDED"] == "Y"):?>up<? else:?>down<?endif ?> icon icon-select_arrow"></i>
                        <?= $arItem["NAME"] ?>
                                    ...
                                    <div class="bx-filter-param-btn-inline">
                                        <? foreach ($arItem["VALUES"] as $val => $ar):?>
                                            ...
                                            <label for="<?= $ar["CONTROL_ID"] ?>"
                                                   data-role="label_<?= $ar["CONTROL_ID"] ?>"
                                                   class="bx-filter-param-label <?= $class ?>"
                                                   onclick="smartFilter.keyup(BX('<?= CUtil::JSEscape($ar["CONTROL_ID"]) ?>')); BX.toggleClass(this, 'bx-active active');">
